...
export class RedisCacheManager<DexIdTypes> {
   ...
   */
  private getRedisKey(dexId: DexIdTypes, namespace: CacheNamespaceType): string {
   ...
   */
  private getPriceKey(token: string): string {
   ...
   */
  async getDexNamespaceRedisCache<T>(
    dexId: DexIdTypes,
   ...
   */
  async setDexNamespaceRedisCache(
    dexId: DexIdTypes,
    namespace: CacheNamespaceType,
   ...
   */
  async deleteDexNamespaceRedisCache(
    dexId: DexIdTypes,
    namespace: CacheNamespaceType
  ): Promise<void> {
    try {
   ...
   */
  async existsDexNamespaceRedisCache(
    dexId: DexIdTypes,
    namespace: CacheNamespaceType
  ): Promise<boolean> {
    try {
   ...
   */
  async getPriceFromRedis(token: string): Promise<number | null> {
    try {
      const key = this.getPriceKey(token);
      const price = (await this.redisClient.get(key)) as string
  ...
  async getBatchPrices(tokens: string[]): Promise<Record<string, number | null>> {
    try {
      ...
      const prices = (await this.redisClient.mGet(keys)) as string[]
