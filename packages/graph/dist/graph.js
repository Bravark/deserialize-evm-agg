...
exports.checkIfGraphIsEmpty = exports.findBestRouteIndex = exports.Edge = void 0;
...
//   return path;
// }
const normalizeDecimals = (value, decimals) => {
    return value / Math.pow(10, decimals);
};
const dijkstraAlgorithmWithKey = (graph, from, to, key, functionToMutateTheEdgeCost) => {
    ...
    queue.enqueue({
        i: from,
        p: functionToMutateTheEdgeCost({ ...graph[from][0].edgeData, key }, graph[from][0]),
    });
    dist[from] = 0;
    while (!queue.isEmpty()) {
 ...
 */
const findBestRouteIndex = (graph, fromIndex, toIndex, key, targetHops, functionToMutateTheEdgeCost) => {
    ...
    if (targetHops !== undefined && finalResult.length - 1 > targetHops) {
        ...
        try {
            ...
            console.warn(`Route reduction failed: ${error.message}`);
...
exports.findBestRouteIndex = findBestRouteIndex;
const calculateTheCost = (path, dist) => {
    let currentCost = 0;
    let pairCost = [];
...
const checkIfGraphIsEmpty = (graph) => {
    ...
    for (let i = 0; i < graph.length; i++) {
        if (graph[i].length > 0)
            nonEmptyNodes++;
        if (nonEmptyNodes > half)
            return false;
    }
    return true;
};
exports.checkIfGraphIsEmpty = checkIfGraphIsEmpty;
const convertPathToPair = (path) => {
    const pairs = [];
 ...
 */
function simulateMultiStepSwap(nodes, initialKey, graph, functionToMutateTheEdgeCost) {
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
        ...
        const result = functionToMutateTheEdgeCost({ ...edge.edgeData, key: initialKey }, edge);
 ...
 */
function reduceRouteByOneHop(route, graph, key, functionToMutateTheEdgeCost) {
    // Ensure that the route has at least four nodes (to reduce to three)
    if (route.length < 4) {
        throw new Error("Route must contain at least four nodes to reduce by one hop.");
    }
    const start = route[0];
    ...
    for (let i = 1; i < route.length - 1; i++) {
        ...
        try {
            ...
            const { totalCost, edges } = simulateMultiStepSwap(candidateRoute, key, graph, functionToMutateTheEdgeCost);
            ...
            console.warn(`Skipping candidate route without node ${route[i]}: ${error.message}`);
            continue;
        }
    }
 ...
 */
function reduceRouteToTargetHops(route, targetHops, graph, key, functionToMutateTheEdgeCost) {
    // Validate input
    if (targetHops < 1) {
    ...
    if (targetHops >= currentHops) {
        // No reduction needed
        const { totalCost, edges } = simulateMultiStepSwap(route, key, graph, functionToMutateTheEdgeCost);
    ...
    while (currentRoute.length - 1 > targetHops) {
        result = reduceRouteByOneHop(currentRoute, graph, key, functionToMutateTheEdgeCost);
...
function simulateTwoStepSwap(start, intermediary, end, initialKey, graph, functionToMutateTheEdgeCost) {
    ...
    const result1 = functionToMutateTheEdgeCost({ ...edge1.edgeData, key: initialKey }, edge1);
 ...
 */
function reduceRouteToTwoHops(route, graph, key, functionToMutateTheEdgeCost) {
    // Ensure that the route has at least three nodes.
    if (route.length < 3) {
        throw new Error("Route must contain at least three nodes (start, intermediary, end).");
    }
    const start = route[0];
    const end = route[route.length - 1];
    ...
    for (let i = 1; i < route.length - 1; i++) {
        ...
        try {
            const { totalCost, edges } = simulateTwoStepSwap(start, candidate, end, key, graph, functionToMutateTheEdgeCost);
            // Update if a better (lower cost) candidate is found.
            if (totalCost < bestOutcome) {
                bestOutcome = totalCost;
            ...
            console.warn(`Skipping candidate ${candidate}: ${error.message}`);
            continue;
        }
    }
